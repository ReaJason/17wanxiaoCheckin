...
def wanxiao_server_push(send_key, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
...
# """
#     )
    return server_push(send_key, "健康打卡", msg)


def wanxiao_email_push(send_email, send_pwd, receive_email, smtp_address, smtp_port, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
#     mail_msg_list = []
...
# """)
    return email_push(send_email, send_pwd, receive_email,
                      title="完美校园健康打卡", text=msg,
                      smtp_address=smtp_address, smtp_port=smtp_port)


def wanxiao_qmsg_push(key, qq_num, type, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
#     utc8_time = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
#     push_list = [
#         f'@face=74@ {utc8_time.strftime("%Y-%m-%d %H:%M:%S")} @face=74@ ']
#     for check_info in msg:
#         if check_info["status"]:
#             name = check_info["post_dict"].get("username")
#             if not name:
...
#             push_list.append(check_info['errmsg'])
    return qmsg_push(key, qq_num, msg, type)


def wanxiao_pipe_push(key, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
#     utc8_time = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
...
#         else:
#             push_list.append(check_info['errmsg'])
    return pipe_push(key, msg)


def wanxiao_wechat_enterprise_push(corp_id, corp_secret, agent_id, to_user, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
    # utc8_time = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
    ...
    #             name = check_info["post_dict"]["name"]
    #         push_list.append(
    #             f"{name}{check_info['type']}：\n{check_info['res']}")
    #     else:
    #         push_list.append(check_info['errmsg'])
    return wechat_enterprise_push(corp_id, corp_secret, agent_id, to_user, msg)


def wanxiao_bark_push(device_key, group, msg):
    '''
        《关于太乱了就摆烂这件事》
    '''
    utc8_time = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
    ...
    #     else:
    #         push_list.append(check_info['errmsg'])
    return bark_push(device_key, msg, title, group)
